    ...
    class QueueManager {
            ...
            try {
                if (!file_exists(self::$driver_filename)) {
                    ...
                    if (!isset($driver_data->running) || !is_array($driver_data->running)) {
                        throw new Exception("Queues file doesnt have a valid running property.");
                    }
                    if (!isset($driver_data->queued) || !is_array($driver_data->queued)) {
                        throw new Exception("Queues file doesnt have a valid queued property.");
                    }
        ...
        public function runQueued() {
            foreach($this->queued as $key => $queue) {
                if ($this->isQueueFree($queue->getQueueName())) {
                    $this->running[] = $queue;
        ...
        private function isQueueFree(string $queue_name) {
            $is_free = true;
